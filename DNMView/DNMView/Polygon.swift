...
public class Polygon: CustomStringConvertible {
    ...
    internal func getDescription() -> String {
        var description: String = "Polygon: "
...
public class ParallelogramVertical: UIBezierPath {
    ...
    internal var left: CGFloat { get { return x - 0.5 * length } }
    internal var right: CGFloat { get { return left + length } }
    ...
    public func getVertices() -> [CGPoint] {
        let y_topLeft: CGFloat = getYAtX(left) - 0.5 * width
        let y_topRight: CGFloat = getYAtX(right) - 0.5 * width
        let y_bottomRight: CGFloat = getYAtX(right) + 0.5 * width
        let y_bottomLeft: CGFloat = getYAtX(left) + 0.5 * width
        
        return [
            CGPointMake(left, y_topLeft),
            CGPointMake(right, y_topRight),
            CGPointMake(right, y_bottomRight),
            CGPointMake(left, y_bottomLeft)
        ]
    }
    
    internal func getYAtX(x: CGFloat) -> CGFloat {
        return y - slope * (x - 0.5 * length)
    }
    
    internal func setPath() {
        let y_topLeft: CGFloat = getYAtX(left) - 0.5 * width
        let y_topRight: CGFloat = getYAtX(right) - 0.5 * width
        let y_bottomRight: CGFloat = getYAtX(right) + 0.5 * width
        let y_bottomLeft: CGFloat = getYAtX(left) + 0.5 * width
        moveToPoint(CGPointMake(left, y_topLeft))
        addLineToPoint(CGPointMake(right, y_topRight))
        addLineToPoint(CGPointMake(right, y_bottomRight))
        addLineToPoint(CGPointMake(left, y_bottomLeft))
        closePath()
    }
}
