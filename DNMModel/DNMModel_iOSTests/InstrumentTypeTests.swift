...
class InstrumentTypeTests: XCTestCase {
    ...
    func testPreferredClefsAndTransposition() {
        let strings: [InstrumentType] = [.Violin, .Viola, .Violoncello]
        for string in strings {
            XCTAssert(string.preferredClefsAndTransposition.count > 0, "string has no preferred clefs and transposition")
        }
        XCTAssert(Strings.Violoncello.preferredClefsAndTransposition.count > 0, "violoncello has no preferred clefs")
    }
